...
if (debug) console.log('[API] API module loaded, debug mode:', debug);
...
function fetchAudio(msg, onSuccess, onError) {
  ...
  fetch(current_tts_endpoint, {
    ...
    .then(arrayBuffer => {
      if (debug) console.log('[API] fetchAudio success, size:', arrayBuffer.byteLength);
...
function fetchChat(msg, prompt, onSuccess, onError, stream = true) {
  ...
  fetch(current_chat_endpoint, {
    ...
    .then(data => {
      if (debug) console.log('[API] fetchChat success');
...
function streamResponseRead(reader, afterGetContent, onDone) {
  ...
  function readChunk() {
    reader.read()
      .then(({ done, value }) => {
        if (done) {
          if (debug) console.log('[API] Stream reading completed');
          if (onDone) onDone();
          return;
        }
        ...
        try {
          ...
          for (const element of dataArray) {
            ...
            if (element === "data: [DONE]") {
              if (debug) console.log('[API] Stream DONE marker received');
              if (onDone) onDone();
              return;
            }
            ...
            try {
              // 移除 'data: ' 前缀（如果存在）
              const jsonStr = element.startsWith('data: ') 
                ? element.substring(6) 
                : element;
