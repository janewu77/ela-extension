# 测试说明

本目录包含 ELA Extension 的测试文件。

## 📋 目录

- [快速开始](#快速开始)
- [运行测试](#运行测试)
- [测试结构](#测试结构)
- [测试覆盖](#测试覆盖)
- [集成测试 API Key 设置](#集成测试-api-key-设置)
- [编写新测试](#编写新测试)
- [常见问题](#常见问题)

## 🚀 快速开始

### 安装依赖

如果还没有安装测试依赖，请先运行：

```bash
npm install
```

### 运行所有测试

```bash
npm test
```

## 🧪 运行测试

### 基本命令

```bash
# 运行所有测试
npm test

# 监听模式（文件变化时自动运行测试）
npm run test:watch

# 生成覆盖率报告
npm run test:coverage
```

### 运行特定测试文件

```bash
# 只运行 background.js 的测试
npm test -- background.test.js

# 只运行 content.js 的测试
npm test -- content.test.js

# 只运行 util.js 的测试
npm test -- util.test.js

# 只运行 storage.js 的测试
npm test -- storage.test.js

# 只运行 options.js 的测试
npm test -- options.test.js

# 只运行 api.js 的测试
npm test -- api.test.js

# 只运行 sidepanel.js 的测试
npm test -- sidepanel.test.js

# 或者使用 Jest 的路径匹配
npm test -- src/__tests__/background.test.js
npm test -- src/__tests__/content.test.js
npm test -- src/__tests__/storage.test.js
npm test -- src/__tests__/options.test.js
npm test -- src/__tests__/api.test.js
npm test -- src/__tests__/sidepanel.test.js
```

### 运行特定测试用例

```bash
# 使用测试名称匹配
npm test -- -t "initStorageValue"

# 运行特定 describe 块
npm test -- -t "存储管理函数"
```

### 调试测试

```bash
# 详细输出模式
npm test -- --verbose

# 只显示失败的测试
npm test -- --onlyFailures

# 更新快照（如果有）
npm test -- -u
```

## 📁 测试结构

### Background.js 测试

测试文件位于 `src/__tests__/background.test.js`，结构如下：

```
Background.js 测试
├── 存储管理函数
│   ├── initStorageValue (6个测试用例)
│   └── initStorageValues (4个测试用例)
├── UI 更新函数
│   └── updateBadge (4个测试用例)
├── 扩展初始化
│   └── initializeExtension (3个测试用例)
└── 事件监听器
    ├── chrome.runtime.onInstalled (2个测试)
    ├── chrome.action.onClicked (1个测试)
    ├── chrome.commands.onCommand (1个测试)
    ├── chrome.storage.local.onChanged (2个测试)
    └── chrome.runtime.onMessage (3个测试)
```

### Content.js 测试

测试文件位于 `src/__tests__/content.test.js`，结构如下：

```
Content.js 测试
├── 状态管理
│   ├── initializeState (4个测试用例)
│   └── updateState (1个测试用例)
├── 文本选择处理
│   ├── getSelectedText (5个测试用例)
│   ├── sendSelectedText (4个测试用例)
│   └── handleTextSelection (3个测试用例)
├── 事件监听器
│   └── handleMouseUp (2个测试用例)
├── 初始化
│   └── initialize (4个测试用例)
└── 集成测试 (2个测试用例)
```

### Util.js 测试

测试文件位于 `src/__tests__/util.test.js`，结构如下：

```
Util.js 测试
├── 字符串处理
│   └── maskMsg (9个测试用例)
│       ├── 掩码长字符串
│       ├── 处理短字符串
│       ├── 处理空值/null/undefined
│       ├── 处理非字符串类型
│       └── 边界情况
└── DOM 操作
    └── calculateLines (17个测试用例)
        ├── 计算单行/多行文本
        ├── 参数验证
        ├── 错误处理
        ├── DOM 清理
        └── 样式复制
```

### Storage.js 测试

测试文件位于 `src/__tests__/storage.test.js`，结构如下：

```
Storage.js 测试
├── 基本 CRUD 操作
│   ├── getStorageValue (6个测试用例)
│   ├── setStorageValue (6个测试用例)
│   ├── removeStorageValue (4个测试用例)
│   └── getStorageValues (4个测试用例)
├── 初始化函数
│   ├── initStorageValue (8个测试用例)
│   └── initStorageValues (6个测试用例)
└── 存储监听器
    └── createStorageListener (8个测试用例)
```

### Options.js 测试

测试文件位于 `src/__tests__/options.test.js`，结构如下：

```
Options.js 测试
├── TTS 配置管理
│   └── initTTSEndpoint (4个测试用例)
├── API Key 管理
│   └── initAPIKey (4个测试用例)
├── TTS 模型和语音
│   ├── initTTSModel (4个测试用例)
│   └── initTTSVoice (4个测试用例)
├── Chat 配置管理
│   ├── initChatEndpoint (4个测试用例)
│   └── initChatModel (4个测试用例)
├── Action Items 管理
│   ├── initActionItems (4个测试用例)
│   └── constructActionItemsHTML (4个测试用例)
├── 辅助函数
│   ├── createRadioOption (4个测试用例)
│   ├── setHelpInfo (4个测试用例)
│   └── resetToDefaults (4个测试用例)
└── 初始化
    └── initializeOptions (4个测试用例)
```

### Api.js 测试

测试文件位于 `src/__tests__/api.test.js`，结构如下：

```
Api.js 测试
├── fetchAudio (6个测试用例)
│   ├── 成功获取音频数据
│   ├── 处理空消息
│   ├── 处理未配置的 endpoint 或 token
│   ├── 处理 HTTP 错误响应
│   ├── 处理 401 错误并显示友好消息
│   └── 处理网络错误
├── fetchChat (7个测试用例)
│   ├── 成功获取流式响应
│   ├── 成功获取非流式响应
│   ├── 处理空消息
│   ├── 处理未配置的 endpoint 或 token
│   ├── 处理 HTTP 错误响应
│   ├── 处理 401 错误并显示友好消息
│   └── 使用默认的 stream 参数
├── streamResponseRead (7个测试用例)
│   ├── 读取流式响应数据
│   ├── 处理 DONE 标记
│   ├── 跳过空行
│   ├── 处理无效的 reader
│   ├── 处理 JSON 解析错误
│   ├── 处理读取错误
│   └── 处理没有 content 的响应
└── 集成测试（默认跳过）
    ├── 应该实际调用 TTS API
    └── 应该实际调用 Chat API
```

### Sidepanel.js 测试

测试文件位于 `src/__tests__/sidepanel.test.js`，结构如下：

```
Sidepanel.js 测试
├── applyConfigData (4个测试用例)
│   ├── 正确应用配置数据
│   ├── 使用默认值当配置数据为 null 或 undefined
│   ├── 处理无效的 action_items
│   └── 处理非数组的 action_items
├── _showOnoff (3个测试用例)
│   ├── 更新 toggleSwitch 的 checked 状态
│   ├── 更新 onoff 元素的文本
│   └── 处理 toggleSwitch 不存在的情况
├── getBtnSetting (1个测试用例)
│   └── 创建设置按钮元素
├── createButton (2个测试用例)
│   ├── 创建按钮元素
│   └── 创建禁用的按钮
├── handleStorageChanges (5个测试用例)
│   ├── 处理 onoff 变化
│   ├── 处理 TTS 配置变化
│   ├── 处理 null/undefined 值，使用默认值
│   ├── 处理 action_items 变化
│   └── 处理无效的 action_items，使用默认值
├── setupStorageListeners (3个测试用例)
│   ├── 设置存储监听器
│   ├── 移除旧的监听器再添加新的
│   └── 处理错误情况
├── initOnoffState (3个测试用例)
│   ├── 通过 init() 初始化 onoff 状态
│   ├── 处理 toggleSwitch 不存在的情况
│   └── 在开关变化时更新存储
├── initConfig (2个测试用例)
│   ├── 调用 getStorageValues 并应用配置
│   └── 处理错误情况
├── initButtons (5个测试用例)
│   ├── 初始化设置按钮
│   ├── 初始化添加内容块按钮
│   ├── 在设置按钮点击时打开选项页
│   ├── 在添加按钮点击时调用 add_content_block
│   └── 处理按钮不存在的情况
├── setupMessageListeners (4个测试用例)
│   ├── 设置消息监听器
│   ├── 处理 selectedText 消息
│   ├── 处理其他类型的消息
│   └── 处理错误情况
└── init (3个测试用例)
    ├── 完成初始化流程
    ├── 在 debug 模式下显示调试信息
    └── 处理初始化错误
```

## 📊 测试覆盖

### 当前测试覆盖

#### Background.js

- ✅ **存储管理函数**：100% 覆盖
  - `initStorageValue`：正常流程、错误处理、边界情况
  - `initStorageValues`：批量操作、并行处理、空配置

- ✅ **UI 更新函数**：100% 覆盖
  - `updateBadge`：ON/OFF 状态、错误处理、多次调用

- ✅ **扩展初始化**：100% 覆盖
  - `initializeExtension`：完整初始化流程、错误处理、配置验证

- ✅ **事件监听器**：注册和基本功能测试

#### Content.js

- ✅ **状态管理**：100% 覆盖
  - `initializeState`：从存储加载、默认值处理、错误处理
  - `updateState`：状态更新

- ✅ **文本选择处理**：100% 覆盖
  - `getSelectedText`：获取选中文本、边界情况、错误处理
  - `sendSelectedText`：发送消息、空文本处理、框架检测
  - `handleTextSelection`：选择处理、扩展状态检查

- ✅ **事件监听器**：100% 覆盖
  - `handleMouseUp`：鼠标事件处理

- ✅ **初始化**：100% 覆盖
  - `initialize`：完整初始化流程、存储监听、事件注册

- ✅ **集成测试**：完整流程测试

#### Util.js

- ✅ **字符串处理**：100% 覆盖
  - `maskMsg`：掩码功能、边界情况、错误处理、非字符串类型

- ✅ **DOM 操作**：100% 覆盖
  - `calculateLines`：行数计算、参数验证、错误处理、DOM 清理、样式复制

#### Storage.js

- ✅ **基本 CRUD 操作**：100% 覆盖
  - `getStorageValue`：正常获取、默认值处理、错误处理
  - `setStorageValue`：正常设置、错误处理、参数验证
  - `removeStorageValue`：正常删除、错误处理
  - `getStorageValues`：批量获取、部分键不存在的情况

- ✅ **初始化函数**：100% 覆盖
  - `initStorageValue`：初始化单个值、默认值处理、错误处理
  - `initStorageValues`：批量初始化、部分失败处理

- ✅ **存储监听器**：100% 覆盖
  - `createStorageListener`：监听单个/多个键、回调触发、移除监听器

#### Options.js

- ✅ **配置管理**：100% 覆盖
  - TTS 配置：endpoint、model、voice
  - Chat 配置：endpoint、model
  - API Key 管理
  - Action Items 管理
  - 辅助函数：创建选项、设置帮助信息、重置默认值
  - 初始化流程

#### Api.js

- ✅ **API 调用**：100% 覆盖
  - `fetchAudio`：成功、错误处理、401 错误、网络错误
  - `fetchChat`：流式/非流式响应、错误处理、401 错误
  - `streamResponseRead`：流式读取、DONE 标记、错误处理、JSON 解析错误

- ⏭️ **集成测试**：默认跳过（需要真实 API key 和网络连接）

#### Sidepanel.js

- ✅ **配置管理**：100% 覆盖
  - `applyConfigData`：应用配置、默认值处理、action_items 处理
  - `handleStorageChanges`：处理各种存储变化、默认值处理

- ✅ **UI 函数**：100% 覆盖
  - `_showOnoff`：更新开关状态和文本
  - `getBtnSetting`：创建设置按钮
  - `createButton`：创建按钮元素

- ✅ **初始化**：100% 覆盖
  - `initOnoffState`：初始化开关状态、事件监听
  - `initConfig`：初始化配置、错误处理
  - `initButtons`：初始化按钮、事件处理
  - `init`：完整初始化流程

- ✅ **消息处理**：100% 覆盖
  - `setupMessageListeners`：设置监听器、处理 selectedText 消息、错误处理

- ✅ **存储监听器**：100% 覆盖
  - `setupStorageListeners`：设置监听器、移除旧监听器、错误处理

### 查看覆盖率报告

```bash
npm run test:coverage
```

报告会生成在 `coverage/` 目录下，打开 `coverage/lcov-report/index.html` 查看详细报告。

## ✍️ 编写新测试

### 测试文件模板

```javascript
describe('功能模块名称', () => {
  beforeEach(() => {
    // 重置所有 mock
    jest.clearAllMocks();
    chrome.storage.local.get.mockClear();
    // ... 其他 mock 重置
  });

  describe('函数名称', () => {
    it('应该描述测试场景', async () => {
      // 1. 准备 (Arrange)
      const input = 'test';
      chrome.storage.local.get.mockResolvedValue({});

      // 2. 执行 (Act)
      const result = await functionToTest(input);

      // 3. 断言 (Assert)
      expect(result).toBe(expected);
    });
  });
});
```

### 常用 Mock 模式

#### Mock Chrome Storage

```javascript
// Mock 成功获取
chrome.storage.local.get.mockResolvedValue({ key: 'value' });

// Mock 获取失败
chrome.storage.local.get.mockRejectedValue(new Error('Storage error'));

// Mock 设置成功
chrome.storage.local.set.mockResolvedValue({});
```

#### Mock Chrome Action

```javascript
// Mock 设置徽章成功
chrome.action.setBadgeText.mockResolvedValue({});

// Mock 设置徽章失败
chrome.action.setBadgeText.mockRejectedValue(new Error('Badge error'));
```

#### Mock Console

```javascript
// 在 beforeEach 中设置
global.console.error = jest.fn();
global.console.log = jest.fn();

// 在测试中验证
expect(console.error).toHaveBeenCalledWith('Error message');
```

### 测试最佳实践

1. **AAA 模式**：Arrange（准备）→ Act（执行）→ Assert（断言）
2. **描述清晰**：使用中文描述测试场景，让测试即文档
3. **独立测试**：每个测试应该独立，不依赖其他测试
4. **清理 Mock**：在 `beforeEach` 中重置所有 mock
5. **测试边界**：测试正常流程、错误情况、边界值

## 🔧 常见问题

### Q: 测试失败，提示 Chrome API 未定义？

**A:** 确保 `jest.setup.js` 中正确配置了 Chrome API mock。检查 `jest.config.js` 中的 `setupFilesAfterEnv` 配置。

### Q: 如何测试异步函数？

**A:** 使用 `async/await`：

```javascript
it('应该处理异步操作', async () => {
  chrome.storage.local.get.mockResolvedValue({});
  const result = await asyncFunction();
  expect(result).toBe(expected);
});
```

### Q: 如何测试事件监听器？

**A:** 由于事件监听器在 `background.js` 中注册，我们主要测试：
1. 监听器是否注册
2. 监听器回调函数的逻辑（提取出来单独测试）

### Q: 测试运行很慢？

**A:** 
- 使用 `--maxWorkers=2` 限制并行数
- 使用 `--runInBand` 串行运行
- 使用 `--watch` 模式只运行变化的测试

### Q: 如何调试单个测试？

**A:** 使用 `--testNamePattern` 或 `-t`：

```bash
npm test -- -t "应该使用默认值"
```

或者在测试中添加 `debugger;` 语句，然后使用 Node.js 调试器。

## 📝 测试命令速查

```bash
# 基本运行
npm test                          # 运行所有测试
npm run test:watch               # 监听模式
npm run test:coverage            # 生成覆盖率

# 过滤测试
npm test -- background.test.js   # 运行特定文件
npm test -- -t "initStorageValue" # 运行匹配的测试

# 调试选项
npm test -- --verbose            # 详细输出
npm test -- --onlyFailures       # 只显示失败
npm test -- --bail               # 遇到失败立即停止
```

## 🔑 集成测试 API Key 设置

`api.test.js` 包含集成测试，默认跳过。要运行集成测试，需要设置 API key。

### 方法 1：使用环境变量（推荐）

#### 在命令行中设置

```bash
# 运行集成测试
SKIP_INTEGRATION_TESTS=false TEST_API_KEY=your-actual-api-key npm test -- src/__tests__/api.test.js

# 或者导出环境变量
export SKIP_INTEGRATION_TESTS=false
export TEST_API_KEY=your-actual-api-key
npm test -- src/__tests__/api.test.js
```

#### 在 package.json 中添加测试脚本

在 `package.json` 的 `scripts` 部分添加：

```json
{
  "scripts": {
    "test:integration": "SKIP_INTEGRATION_TESTS=false TEST_API_KEY=$TEST_API_KEY jest src/__tests__/api.test.js"
  }
}
```

然后运行：
```bash
TEST_API_KEY=your-actual-api-key npm run test:integration
```

### 方法 2：使用 .env 文件（需要 dotenv）

#### 安装 dotenv

```bash
npm install --save-dev dotenv
```

#### 创建 .env.test 文件

```bash
# .env.test
SKIP_INTEGRATION_TESTS=false
TEST_API_KEY=your-actual-api-key
```

#### 修改 jest.setup.js

在 `jest.setup.js` 开头添加：

```javascript
require('dotenv').config({ path: '.env.test' });
```

### 方法 3：直接在测试代码中设置（不推荐，但方便快速测试）

在 `api.test.js` 的 `beforeAll` 中直接设置：

```javascript
beforeAll(() => {
  if (SKIP_INTEGRATION) {
    return;
  }
  
  // 直接在这里设置 API key（仅用于测试）
  global.current_auth_token = 'your-actual-api-key-here';
});
```

### 方法 4：使用系统环境变量

在 `~/.bashrc` 或 `~/.zshrc` 中添加：

```bash
export TEST_API_KEY=your-actual-api-key
```

然后运行测试：

```bash
SKIP_INTEGRATION_TESTS=false npm test -- src/__tests__/api.test.js
```

### 注意事项

1. **不要提交 API key 到版本控制**
   - 确保 `.env.test` 在 `.gitignore` 中
   - 不要在代码中硬编码 API key

2. **使用测试专用的 API key**
   - 建议使用单独的测试 API key，而不是生产环境的 key
   - 设置 API key 的使用限额

3. **集成测试默认跳过**
   - 集成测试使用 `describe.skip`，默认不会运行
   - 要运行集成测试，需要：
     - 设置 `SKIP_INTEGRATION_TESTS=false`
     - 设置有效的 `TEST_API_KEY`

4. **Node.js 版本要求**
   - Node.js 18+ 内置了 `fetch`，不需要额外安装
   - Node.js < 18 需要安装 `node-fetch`: `npm install node-fetch`

### 运行集成测试

```bash
# 方法 1：命令行设置
SKIP_INTEGRATION_TESTS=false TEST_API_KEY=your-key npm test -- src/__tests__/api.test.js

# 方法 2：如果已设置环境变量
SKIP_INTEGRATION_TESTS=false npm test -- src/__tests__/api.test.js
```

## 🎯 下一步

- [x] 添加集成测试（已完成，默认跳过）
- [ ] 添加 E2E 测试（使用 Puppeteer）
- [ ] 提高代码覆盖率到 90%+
- [ ] 添加性能测试
- [ ] 设置 CI/CD 自动运行测试

## 📚 相关资源

- [Jest 官方文档](https://jestjs.io/docs/getting-started)
- [Chrome Extension API 文档](https://developer.chrome.com/docs/extensions/reference/)
- [测试最佳实践](https://github.com/goldbergyoni/javascript-testing-best-practices)

---

**最后更新**: 2025-01-13  
**测试框架**: Jest 29.7.0  
**测试环境**: Node.js

## 📦 测试文件列表

当前测试文件：

- ✅ `background.test.js` - Background 脚本测试
- ✅ `content.test.js` - Content 脚本测试
- ✅ `util.test.js` - 工具函数测试
- ✅ `storage.test.js` - 存储管理模块测试
- ✅ `options.test.js` - 选项页面测试
- ✅ `api.test.js` - API 调用测试
- ✅ `sidepanel.test.js` - 侧边栏主文件测试
- 📝 `mockConst.mock.js` - Mock 常量文件（非测试文件）
